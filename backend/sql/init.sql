...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `phone` VARCHAR(30) DEFAULT NULL COMMENT '手机号，可选',
...
CREATE TABLE IF NOT EXISTS `user_sessions` (
  ...
  `refresh_token` VARCHAR(255) DEFAULT NULL COMMENT '刷新token，可选',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='导航分组表';
...
CREATE TABLE IF NOT EXISTS `nav_links` (
  ...
  `icon_url` VARCHAR(500) DEFAULT NULL COMMENT '图标地址或 favicon 地址',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='导航链接表';

-- 用户设置表（建议加上，为主题、跳转模式、首页偏好预留）
